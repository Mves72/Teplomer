class Dial
{
  ...
  void display (int iVal)
  {
    ...
    fill (150);
    ellipse(posX, posY, radius*1.3, radius*1.3);//vnitrni kruh
    fill(00, 00, 200);
    textFont(f2);
    text(face, posX, posY - radius/4);//nadpis
    textFont(f);
    for (int i = 0 ; i <= 10 ; i++) {
      stroke (0);
      strokeWeight(2);
    ...
    fill(255, 0, 0);
    strokeWeight(2);
    ellipse(posX, posY, radius*0.1, radius*0.1);//osa rafiky
    stroke(255,0, 0);
    line(posX, posY, posX + cos(radians) * radius * 0.7, posY + sin(radians) * radius * 0.7);//rafika
    fill(00, 00, 150);
    textFont(f2);
    text(val, posX, posY+radius/4);//
  }
}